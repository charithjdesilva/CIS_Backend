from fastapi import APIRouter, HTTPException, Form, status, UploadFile, Path, File
...
async def update_crime(
    ...
    if crime:
        crime_photo = db.query(Photos).filter(
            Photos.PhotoID == crime.CrimeID).first()
        if crime_photo is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"crime is not found")
    else:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"crime is not found")
...
@router.patch('/update/crime/add_more_photos/{id}')
async def update_crime_with_add_more_photos(
    db: db_dependency,
    id: Annotated[str, Path(description="Enter Crime ID : ")],
    photos_crime: List[UploadFile] = File(...)
):
    if not photos_crime or type(photos_crime) == str:
        raise HTTPException(status_code=status.HTTP_200_OK,
                            detail="crime photos not included")
    crime = db.query(Crime).filter(Crime.CrimeID == id).first()

    if crime:
        crime_photos = db.query(Photos).filter(
            Photos.PhotoType == "Crime").all()

        # Filter photos where PhotoID contains CrimeID
        filtered_photos = [
            photo for photo in crime_photos if crime.CrimeID in photo.PhotoID]

        # Get the count of filtered photos
        count_filtered_photos = len(filtered_photos)

        for photo in photos_crime:
            upload_img_url = await upload_image_with_multiple_photos(photo, id, UPLOAD_CRIME, count_filtered_photos)
            img_url = upload_img_url
            photo = Photos(
                PhotoID=f"{id}%{count_filtered_photos}",
                PhotoType="Crime",
                PhotoPath=img_url
            )
...
async def update_victim(
    ...
    victim_details = db.query(Person).filter(
        Person.PersonID == victim_id).first()

    if victim_details:
        victim_photo = db.query(Photos).filter(
            Photos.PhotoID == victim_details.PersonID).first()
        if victim_photo is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"victim is not found")
    else:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"victim is not found")
...
@router.patch('/update/victim/add_more_photos/{id}')
async def update_victim_with_add_more_photos(
    db: db_dependency,
    id: Annotated[str, Path(description="Enter Victim ID : ")],
    photos_victim: List[UploadFile] = File(...)
):
    if not photos_victim or type(photos_victim) == str:
        raise HTTPException(status_code=status.HTTP_200_OK,
                            detail="crime photos not included")
    victim = db.query(Person).filter(Person.PersonID == id).first()

    if victim:
        victim_photos = db.query(Photos).filter(
            Photos.PhotoType == "Victim").all()

        # Filter photos where PhotoID contains CrimeID
        filtered_photos = [
            photo for photo in victim_photos if victim.PersonID in photo.PhotoID]

        # Get the count of filtered photos
        count_filtered_photos = len(filtered_photos)

        for photo in photos_victim:
            upload_img_url = await upload_image_with_multiple_photos(photo, id, UPLOAD_VICTIM, count_filtered_photos)
            img_url = upload_img_url
            photo = Photos(
                PhotoID=f"{id}%{count_filtered_photos}",
                PhotoType="Victim",
...
async def update_evidence(
    ...
    evidence_details = db.query(Evidence).filter(
        Evidence.EvidenceID == evidence_id).first()

    if evidence_details:
        evidence_photo = db.query(Photos).filter(
            Photos.PhotoID == evidence_details.EvidenceID).first()
        if evidence_photo is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"evidence is not found")
    else:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"evidence is not found")
...
async def register_crim_suspect(
    ...
    criminal_person = db.query(Person).filter(
        Person.PersonID == PersonID).first()
    criminal_sus = db.query(CriminalOrSuspect).filter(
        CriminalOrSuspect.PersonID == PersonID).first()

    if criminal_person and criminal_sus:
        criminal_sus_photo = db.query(Photos).filter(
            Photos.PhotoID == criminal_person.PersonID).first()
        if criminal_sus_photo is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"criminal/suspect is not found")
    else:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"criminal/suspect is not found")
